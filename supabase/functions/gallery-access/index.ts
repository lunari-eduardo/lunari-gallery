...
serve(async (req) => {
  ...
  try {
    ...
    if (gallery.status_selecao === 'aguardando_pagamento') {
      ...
      // SAFETY: Only auto-heal if there's NO newer pending cobrança (which means a new cycle started)
      ...
      if (!newerPending) {
        ...
        if (paidCobranca) {
          ...
          if (healResult?.gallery_synced || healResult?.already_paid) {
            console.log('✅ Auto-heal gallery-access: galeria sincronizada');
        ...
        console.log('⚠️ AUTO-HEAL skipped: newer pending cobrança exists for gallery', gallery.id);
      }
    }
    ...
    // `regras_congeladas.pacote.valorFotoExtra` (vindo da sessão) prevalece
    // sempre. A coluna escalar `gallery.valor_foto_extra` é apenas um espelho
    // visual para listagens — não é mais reconciliada aqui.
