...
-- Fix finalize_gallery_payment: also finalize visitor when cobranca has visitor_id
...
-- Backfill: finalize visitors that have paid cobrancas but are still em_andamento
UPDATE public.galeria_visitantes v
SET status = 'finalizado',
    status_selecao = 'selecao_completa',
    finalized_at = c.data_pagamento,
    updated_at = now()
