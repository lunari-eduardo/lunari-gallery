...
-- 1) Coluna para armazenar o slug do invoice InfinitePay (permite fetch pós-pagamento)
ALTER TABLE public.cobrancas
  ADD COLUMN IF NOT EXISTS ip_invoice_slug text;

CREATE INDEX IF NOT EXISTS idx_cobrancas_ip_invoice_slug
  ON public.cobrancas (ip_invoice_slug)
  WHERE ip_invoice_slug IS NOT NULL;

-- 2) RPC pública para o Gallery gravar contato do visitante antes do checkout.
--    - Só grava colunas vazias em `clientes` (mesmo contrato do enrichClienteIfMissing).
--    - Sempre atualiza `galeria_visitantes` quando visitor_id é dono da linha.
--    - Retorna quais colunas foram enriquecidas.
...
BEGIN
  ...
  -- resolve galeria + cliente pelo token (com fallback via aliases)
  SELECT id, cliente_id
    INTO v_gallery_id, v_cliente_id
  FROM public.galerias
  WHERE public_token = p_token
  LIMIT 1;

  IF v_gallery_id IS NULL THEN
    SELECT g.id, g.cliente_id
      INTO v_gallery_id, v_cliente_id
    FROM public.gallery_token_aliases a
    JOIN public.galerias g ON g.id = a.galeria_id
    WHERE a.alias_token = p_token
    LIMIT 1;
  END IF;
  ...
  -- Atualiza visitante (se pertencer à galeria)
           ...
           contato_tipo = CASE
             WHEN v.contato IS NULL OR trim(v.contato) = ''
               THEN CASE WHEN v_email IS NOT NULL THEN 'email'
                         WHEN v_phone_digits IS NOT NULL THEN 'telefone'
                         ELSE v.contato_tipo END
             ELSE v.contato_tipo
           END,
           updated_at = now()
     WHERE v.id = p_visitor_id
       AND v.galeria_id = v_gallery_id;

    IF FOUND THEN
      v_enriched_visitor := true;
    END IF;
  END IF;

  -- Enriquecer cadastro do cliente (só grava colunas vazias)
  IF v_cliente_id IS NOT NULL THEN
    SELECT email, telefone, whatsapp, nome
      INTO v_current
...
REVOKE ALL ON FUNCTION public.upsert_visitor_contact(text, uuid, text, text, text) FROM public;
GRANT EXECUTE ON FUNCTION public.upsert_visitor_contact(text, uuid, text, text, text) TO anon, authenticated, service_role;
