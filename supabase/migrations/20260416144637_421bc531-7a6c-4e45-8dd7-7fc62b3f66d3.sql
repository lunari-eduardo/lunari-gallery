...
-- Re-apply: Fix finalize_gallery_payment to also finalize visitor when cobranca has visitor_id
...
BEGIN
    ...
    IF v_has_parcelas THEN
      ...
      IF v_current_status IN ('pago', 'pago_manual') THEN
        IF v_galeria_id IS NOT NULL THEN
          ...
          END IF;
        END IF;
        -- VISITOR FINALIZATION (parcelas resolved path)
        IF v_cobranca.visitor_id IS NOT NULL THEN
          UPDATE public.galeria_visitantes
          SET status = 'finalizado',
              status_selecao = 'selecao_completa',
  ...
  -- VISITOR FINALIZATION (new payment path)
  IF v_cobranca.visitor_id IS NOT NULL THEN
    UPDATE public.galeria_visitantes
    SET status = 'finalizado',
        status_selecao = 'selecao_completa',
...
-- Backfill: finalize visitors that have paid cobrancas but are still not finalized
UPDATE public.galeria_visitantes v
SET status = 'finalizado',
    status_selecao = 'selecao_completa',
    finalized_at = COALESCE(c.data_pagamento, now()),
    updated_at = now()
...
-- Backfill: for cobrancas WITHOUT visitor_id, try to match by galeria_id 
-- when there's only ONE visitor in aguardando_pagamento for that gallery
UPDATE public.cobrancas c
SET visitor_id = v.id
FROM public.galeria_visitantes v
WHERE c.galeria_id = v.galeria_id
  AND c.visitor_id IS NULL
  AND c.status IN ('pago', 'pago_manual')
  AND v.status_selecao = 'aguardando_pagamento'
  AND (SELECT count(*) FROM public.galeria_visitantes 
       WHERE galeria_id = c.galeria_id AND status_selecao = 'aguardando_pagamento') = 1;

-- Now finalize those newly linked visitors
UPDATE public.galeria_visitantes v
SET status = 'finalizado',
    status_selecao = 'selecao_completa',
    finalized_at = COALESCE(c.data_pagamento, now()),
