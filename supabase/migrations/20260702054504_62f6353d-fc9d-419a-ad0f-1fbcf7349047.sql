-- Adiciona parâmetro p_cpf_cnpj à RPC upsert_visitor_contact.
-- Grava CPF/CNPJ em clientes.cpf_cnpj APENAS se estiver NULL (nunca sobrescreve).
-- Valida dígitos verificadores? Não — validação forte fica no frontend + Asaas.
-- Aqui só valida comprimento (11 ou 14 dígitos). Se inválido, ignora silenciosamente.
