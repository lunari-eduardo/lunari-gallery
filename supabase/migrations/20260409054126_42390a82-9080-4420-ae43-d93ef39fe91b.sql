...
-- RPC: Lock por visitante para galerias públicas
...
BEGIN
  ...
  IF v_visitor.status = 'finalizado' THEN
    -- Allow retry if visitor was finalized but has a pending charge > 24h
...
-- Fix retroactive test data
UPDATE galeria_visitantes SET fotos_selecionadas = 6, status = 'finalizado', status_selecao = 'selecao_completa', finalized_at = NOW() WHERE id = '4f7ee6bf-3641-4d8e-93ea-7b9f5d20291a';
UPDATE galeria_visitantes SET fotos_selecionadas = 3, status = 'finalizado', status_selecao = 'selecao_completa', finalized_at = NOW() WHERE id = 'ae688254-7c1d-4787-8698-0f75d733c538';
