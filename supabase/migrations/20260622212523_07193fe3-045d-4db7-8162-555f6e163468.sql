-- 1. Fix the finalize_gallery_payment function: use NULLIF on snapshot
...
BEGIN
  ...
  IF v_cobranca.status IN ('pago','pago_manual') THEN
    IF v_galeria_id IS NOT NULL THEN
      -- NULLIF(snapshot,0): trata snapshot=0 como "não informado" para evitar contar todas as fotos como extras
      SELECT GREATEST(COALESCE(g.fotos_selecionadas,0) - COALESCE(NULLIF(v_cobranca.snapshot_fotos_incluidas, 0), g.fotos_incluidas, 0), 0)
...
-- 2. Normalize existing snapshot=0 to NULL so future recalcs work
UPDATE public.cobrancas
SET snapshot_fotos_incluidas = NULL
WHERE snapshot_fotos_incluidas = 0;

-- 3. Reconcile galleries that have paid cobrancas: recompute extras vendidas and valor vendido
WITH agg AS (
  SELECT
    c.galeria_id,
...
SET total_fotos_extras_vendidas = GREATEST(COALESCE(g.fotos_selecionadas,0) - COALESCE(g.fotos_incluidas,0), 0),
    valor_total_vendido = a.sum_val,
    updated_at = now()
FROM agg a
WHERE g.id = a.galeria_id
  AND (g.total_fotos_extras_vendidas IS DISTINCT FROM GREATEST(COALESCE(g.fotos_selecionadas,0) - COALESCE(g.fotos_incluidas,0), 0)
       OR g.valor_total_vendido IS DISTINCT FROM a.sum_val);